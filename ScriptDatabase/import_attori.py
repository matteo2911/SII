...
CSV_PATH = project_root / 'data' / 'attori.csv'

sys.path.append(str(project_root))

from flask import Flask
from database import Attore, db, Regista
...
def importa_attori():
    ...
    with app.app_context():
        try:
            ...
            print("\n[INFO] Inizio importazione attori...")
            ...
            with open(CSV_PATH, 'r', encoding='utf-8-sig') as file:
                ...
                if not id_header or not name_header:
                    ...
                    print("Esempio atteso: attore_id,nome_attore")
                    return
                ...
                for riga in csv_reader:
                    ...
                    try:
                        ...
                        attore_id = int(riga[id_header].strip())
                        nome = riga[name_header].strip()

                        # Verifica se esiste già
                        if db.session.get(Attore, attore_id):
                            print(f"[WARN] Regista ID {attore_id} già esistente, salto")
                            errori += 1
                            continue

                        # Crea nuovo regista
                        db.session.add(Attore(
                            id_attore=attore_id,
                            nome=nome
                        ))
                        successi += 1
                ...
                print(f"Attori importati: {successi}")
